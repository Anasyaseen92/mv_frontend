...
function ProductsPage() {
  ...
  useEffect(() => {
    if (!categoryData) {
      ...
      const sorted = allProducts ? [...allProducts].sort((a, b) => b.total_sell - a.total_sell) : [];
      setData(sorted);
    } else {
      // Filter by selected category
      const filtered = allProducts ? allProducts.filter(
    ...
    <div>
      ...
      <div className={`${styles.section}`}>
        <div className='grid grid-cols-1 gap-[20px] md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 mb-12'>
          {data && data.map((i, index) => <ProductCard data={i} key={i._id || index} />)}
        </div>
